...
func Serve(router *mux.Router, store *storage.Storage, pool *worker.Pool, feedHandler *feed.Handler) {
	...
	// Entry editing pages.
	uiRouter.HandleFunc("/entry/create", handler.showAddEntryPage).Name("addEntry").Methods("GET")
	uiRouter.HandleFunc("/entry/create", handler.submitEntry).Name("submitEntry").Methods("POST")
	uiRouter.HandleFunc("/entry/{entryID}/edit", handler.showEditEntryPage).Name("editEntry").Methods("GET")
	uiRouter.HandleFunc("/entry/update", handler.updateEntry).Name("updateEntry").Methods("POST")
	uiRouter.HandleFunc("/entry/bookmarklet", handler.bookmarkletEntry).Name("bookmarkletEntry").Methods("GET")
